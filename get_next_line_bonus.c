...
char	*get_next_line(int fd)
{
	t_list			*current;
	char			*line;
	static t_list	*heads[FOPEN_MAX];

	if (!gnl_check_head(&heads[fd], fd))
		return (NULL);
	if (heads[fd]->nl > -1)
		return (gnl_make_line(&heads[fd]));
	current = heads[fd];
	while (current && current->nl == -1)
	{
		current->next = gnl_new_node(fd);
		if (!current->next)
			return (gnl_free_list(&heads[fd]));
		current = current->next;
		if (current->nl == -2)
		{
			line = gnl_make_line(&heads[fd]);
			if (!line)
				return (gnl_free_list(&heads[fd]));
			gnl_free_list(&heads[fd]);
			return (line);
		}
	}
	return (gnl_make_line(&heads[fd]));
